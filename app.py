...
from build_database import init_db, process_ride
...
def _reload():
    """Re-read rides and mmp from the DB and bump the version counter."""
    global _rides, _mmp_all, _data_version
    r = _load_rides()
    m = _load_mmp_all(r)
    with _lock:
        _rides        = r
...
def get_data():
    """Return a consistent (version, rides, mmp_all) snapshot."""
    with _lock:
